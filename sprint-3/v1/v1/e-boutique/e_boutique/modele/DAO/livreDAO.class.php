<?php
	...
	include_once(__DIR__ . '/../livre.class.php');
	...
	class LivreDAO implements DAO {	
		...
		static public function modifier($unLivre) {
			...
			$requete = $connexion->prepare("UPDATE livre SET titre=?, auteur=?, description=?, date_publication=?, nbr_pages=?, editeur=?, edition=?, isbn10=?, isbn13=?, prix=?, categorie=?, label=?, chemin_img=?, quantite=? WHERE numero=?");
		
			$tableauInfos = [
				$unLivre->getTitre(),
				$unLivre->getAuteur(),
				$unLivre->getDescription(),
				$unLivre->getDatePublication(),
				$unLivre->getNbrPages(),
				$unLivre->getEditeur(),
				$unLivre->getEdition(),
				$unLivre->getIsbn10(),
				$unLivre->getIsbn13(),
				$unLivre->getPrix(),
				$unLivre->getCategorie(),
				$unLivre->getLabel(),
				$unLivre->getCheminImg(),
				$unLivre->getQuantite(),
				$unLivre->getNumero(),
			];
